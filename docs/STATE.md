...
### ⚠️ Вопрос: возможная инверсия направления лотка в `motors.move_tray`
`move_tray` ставит `extend → TRAY_DIR=1`, `retract → TRAY_DIR=0`. Но config и полевой
`tray_calib_final.py` трактуют `DIR=0=FRONT`, `DIR=1=BACK`. Если «extend» (лоток наружу,
к ячейке) физически = FRONT, то в `move_tray` extend/retract ИНВЕРТИРОВАНЫ. НЕ менял —
это либо реальный баг, либо «extend» в этой механике действительно идёт к BACK.
**Проверить на сухом цикле выдачи** (наблюдать, в какую сторону едет лоток на extend), потом править при необходимости.
